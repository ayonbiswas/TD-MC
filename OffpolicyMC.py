...
class MC_offpolicy:
	...
	def __init__(self, env, num_episodes ,behavior_policy, discount_factor=1.0 ):
		self.env =env
...
# def target_policy(Q,state,action):
	...
	def update(self,episode):
		...
		for t in range(len(episode))[::-1]:
			...
			self.Q[state][action] += (W /( C[state][action]+1e-60)) * (G - Q[state][action])
	...
	def simulate(self,render= False,train = False,verbose= False):
	...
	np.save("./rewards/t_off_MC_{}.npy".format(numTrials), t_Rewards)
	np.save("./rewards/b_off_MC_{}.npy".format(numTrials), b_Rewards)
...
rl =  MC_offpolicy(env, num_episodes=20000, behavior_policy=random_policy)
Q, target_policy = rl.simulate()
